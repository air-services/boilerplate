...
class CrudGetItemView:
    ...
    def get_item_view(self):
        async def item_view(item_id: int):
            ...
            relations_data = await self.get_item_relations_data(item)
            return {**item_data, **relations_data}

        return item_view

    async def get_item_relations_data(self, item):
        result = {}
        for relation in self.relations.get_item_relations:
            ...
            if relation.relation_type == CrudModelRelationType.CHILDREN:
                relation_data = await self._get_children_item_relations(
                    item_id=item.id,
                    relation_model=relation.relation_model,
            ...
            if relation.relation_type == CrudModelRelationType.PARENT:
                relation_data = await self._get_parent_item_relations(
    ...
    @staticmethod
    async def _get_children_item_relations(item_id, relation_model, base_key):
        items = await relation_model.query.where(
            getattr(relation_model, base_key) == item_id
        ).gino.all()
        return [item.to_dict() for item in items]

    async def _get_parent_item_relations(
        self, item, relation: CrudModelRelation
    ):
